...
insert ∷ (Base ⨯ Tree Base) → Tree Base | Tree Base [(x^1,2) |-> 1/2, (x^1) ↦ 1/2, x ↦ 1, (2) |-> 3] → Tree Base [e1 ↦ 1]
insert a x = (meld (node leaf a leaf) x)

delete_min ∷ Tree Base → Tree Base | Tree Base [(x^1) |-> 3/2, x ↦ 1] → Tree Base [e1 ↦ 1]
delete_min x = match x with
  | leaf       → leaf
  | node t a u → meld t u

{-# NUM_CF_SIGS 2 #-}
meld ∷ (Tree Base ⨯ Tree Base) → Tree Base | Tree Base [x ↦ 1, y ↦ 1, (x^1,y^1,-1) ↦ 1/2, (x^1) |-> 1/2, (y^1) |-> 1/2] → Tree Base [e1 ↦ 1] 
