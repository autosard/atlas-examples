...
insert ∷ (Base ⨯ Tree Base) → Tree Base @> Tree Base [(x^1) |-> 1, (2) |-> 1]
insert a x = (meld (node leaf a leaf) x)

delete_min ∷ Tree Base → Tree Base @> Tree Base [(x^1) |-> 2]
delete_min x = match x with
  | leaf       → leaf
  | node t a u → meld t u

meld :: (Tree Base * Tree Base) -> Tree Base @> Tree Base [(x^1) ↦ 1, (y^1) ↦ 1] 
